...
void CGGrEncoder::setState(CGGrState* state) {
  _impl->encoding
    .push_back(make_unique<CGStateGrCmd>(state));
}

void CGGrEncoder::setViewport(CGViewport viewport, uint32_t viewportIndex) {
  _impl->encoding
    .push_back(make_unique<CGViewportCmd>(viewport, viewportIndex));
}

void CGGrEncoder::setScissor(CGScissor scissor, uint32_t viewportIndex) {
  _impl->encoding
    .push_back(make_unique<CGScissorCmd>(scissor, viewportIndex));
}

void CGGrEncoder::setTarget(CGTarget* target) {
  _impl->encoding
    .push_back(make_unique<CGTargetCmd>(target));
}

void CGGrEncoder::setDcTable(uint32_t tableIndex, uint32_t allocIndex) {
  _impl->encoding
    .push_back(make_unique<CGDcTableCmd>(tableIndex, allocIndex));
}
...
void CGGrEncoder::setVertexBuffer(CGBuffer* buffer,
                                  ...
                                  uint32_t inputIndex) {
  _impl->encoding
    .push_back(make_unique<CGVxBufferCmd>(buffer, offset, inputIndex));
}
...
void CGGrEncoder::setIndexBuffer(CGBuffer* buffer,
                                 ...
                                 CGIndexType type) {
  _impl->encoding
    .push_back(make_unique<CGIxBufferCmd>(buffer, offset, type));
}
...
void CGGrEncoder::draw(uint32_t vertexStart,
  ...
  _impl->encoding
    .push_back(make_unique<CGDrawCmd>(vertexStart, vertexCount,
...
void CGGrEncoder::drawIndexed(uint32_t indexStart,
                              ...
                              uint32_t instanceCount) {
  _impl->encoding
    .push_back(make_unique<CGDrawIxCmd>(indexStart, vertexCount, vertexOffset,
                                        baseInstance, instanceCount));
}

void CGGrEncoder::clearColor(CGColor value, uint32_t colorIndex) {
  _impl->encoding
    .push_back(make_unique<CGClearClCmd>(value, colorIndex));
}

void CGGrEncoder::clearDepth(float value) {
  _impl->encoding
    .push_back(make_unique<CGClearDpCmd>(value));
}

void CGGrEncoder::clearStencil(uint32_t value) {
  _impl->encoding
    .push_back(make_unique<CGClearScCmd>(value));
}
...
void CGCpEncoder::setState(CGCpState* state) {
  _impl->encoding
    .push_back(make_unique<CGStateCpCmd>(state));
}
...
void CGCpEncoder::dispatch(CGSize3 size) {
  _impl->encoding
    .push_back(make_unique<CGDispatchCmd>(size));
}
